...
interface ResolvedConfig {
  ...
  tz: string;
  enabled: () => boolean;
  alertOnce: ((kind: ErrorKind, name: string, errMsg: string) => Promise<void> | void) | null;
}
...
const _syncedThisProcess = new Set<string>();
const _alertedKinds = new Set<ErrorKind>();

export function configure(opts: CatalogConfig): void {
  _config = {
    token: opts.token || "",
    dbId: opts.dbId || "",
...
export function _detectSourceFile(): string {
  const e = new Error();
  const stack = (e.stack || "").split("\n").slice(2);
  for (const line of stack) {
    if (line.includes("scheduler-runtime")) continue;
    if (line.includes("job-manager")) continue;
    const m = line.match(/\(([^)]+):\d+:\d+\)$/) || line.match(/at ([^\s]+):\d+:\d+$/);
    if (m && m[1]) {
      const abs = m[1];
      const repoIdx = abs.indexOf("/src/app/");
      if (repoIdx !== -1) return "src/" + abs.slice(repoIdx + 5);
      const serverIdx = abs.indexOf("/server.js");
      if (serverIdx !== -1) return "server.js";
      return abs;
    }
  }
  return "";
...
async function _fetchExistingRow(name: string): Promise<NotionPage | null> {
  if (!_config) throw new Error("not configured");
  ...
  const j = (await resp.json()) as { results?: NotionPage[] };
  return (j.results || [])[0] || null;
}

async function _patchPage(pageId: string, properties: Record<string, unknown>): Promise<unknown> {
  if (!_config) throw new Error("not configured");
...
async function _createPage(properties: Record<string, unknown>): Promise<unknown> {
  if (!_config) throw new Error("not configured");
  const dbNorm = String(_config.dbId).replace(/-/g, "");
...
export function _classifyError(err: unknown): ErrorKind {
  const msg = err instanceof Error ? err.message : String(err || "");
  if (msg.includes("404")) return "db_not_found_or_unshared";
  if (msg.includes("401") || msg.includes("403")) return "auth_denied";
  if (msg.includes("429")) return "rate_limited";
  if (msg.includes("ENOTFOUND") || msg.includes("ECONN") || msg.includes("ETIMEDOUT")) return "network";
  if (/\b5\d\d\b/.test(msg)) return "notion_5xx";
  return "unknown";
}

async function _maybeAlert(kind: ErrorKind, name: string, errMessage: string): Promise<void> {
  if (_alertedKinds.has(kind)) return;
...
export async function syncSchedulerToCatalog(req: SyncRequest): Promise<void> {
  const { name, cron, sourceFile } = req;
  if (!isConfigured() || !_config) return;
  if (!_config.enabled()) return;
  if (_syncedThisProcess.has(name)) return;
